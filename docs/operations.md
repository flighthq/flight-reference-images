...
The array is the complete membership for that Flight run. It must contain 1–256 candidates with unique request paths and artifact ids. The serialized GitHub `client_payload` must also remain within GitHub's 65,535-character limit. Batch intake validates the whole envelope, serializes competing batch runs, and warns away request ids already on `main` or in an open approval PR before starting candidates. It then prepares the remaining candidates independently with `fail-fast: false` and at most 12 concurrent reusable intake calls. One scoped writer verifies every complete prepared/review artifact pair and opens one stable `approval-batch/<flight-run-id>` PR containing all successful approval records. Missing, invalid, and late-merged candidates are warnings rather than batch-wide failures; Flight's next run may safely redispatch them.
...
A version-2 batch approval PR changes one `approvals/<request-id>.json` per successfully admitted candidate, and merging it blesses that PR's set atomically. Its run summary warns about candidates already accepted, under review, or deferred for a later retry. Legacy single-candidate approval PRs change one record. Separate PRs with disjoint targets can be merged in any order; no refresh, draft promotion, special ordering, or textual conflict resolution is part of the normal path. Merged approvals accumulate in one `publication/*` PR. Merge that mechanical PR whenever the desired group is ready; its CI reconstructs and verifies the complete release.
