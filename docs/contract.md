...
Batch membership and approval are atomic, while candidate preparation remains independently observable and retryable. The matrix uses `fail-fast: false` with bounded parallelism, so one invalid candidate does not cancel the diagnostics for unrelated work, but the batch writer runs only after every candidate succeeds. Each candidate crosses a separate read-only decoding job; one scoped writer then revalidates the complete prepared-artifact set and writes all `approvals/<request-id>.json` records into a stable, source-run-keyed batch PR. Retrying the dispatch lease-replaces that PR instead of opening duplicates. The legacy `flight-reference-image-candidate` version-1 event remains accepted during migration and retains its stable request-keyed approval PR.
