...
The completion job writes Flight's version-2 [`reference-image-lock.schema.json`](../schemas/reference-image-lock.schema.json) shape to `scripts/reference-image-lock.json` and removes every request fulfilled by the batch in the same PR. If a Flight lock-update PR is already open, later releases first rebase that bot-owned branch onto the current Flight base, then advance it and accumulate its fulfilled-request removals instead of opening conflicting siblings. Rebinding to the current base cannot silently delete a changed request: the exact request checksum recorded at intake is verified again after the rebase. Each pack entry pins both the encoded pack and an identity-keyed `images` map of decoded `pixelSha256` values, so Flight can decide whether a captured image is already represented without downloading the pack. Required `referenceImage` coverage stays in Flight throughout.
