...
The dispatch membership is validated atomically, while candidate admission is best-effort, independently observable, and retryable. Batches are serialized, and requests already merged or already present in an open approval PR are warned and removed before fan-out. The matrix uses `fail-fast: false` with bounded parallelism, so one invalid candidate does not cancel unrelated work. Each remaining candidate crosses a separate read-only decoding job; one scoped writer revalidates every complete prepared/review artifact pair and writes the successful `approvals/<request-id>.json` records into a stable, source-run-keyed batch PR. Missing or invalid members are warned and deferred for a later Flight run. A final main-branch check also turns an approval merged during intake into a harmless warning instead of invalidating the batch. If a retry recovers omissions while the source run's batch PR remains open, it preserves that reviewed PR and opens a disjoint supplemental PR for only the recovered records. The legacy `flight-reference-image-candidate` version-1 event remains accepted during migration and retains its stable request-keyed approval PR; a request already on main is likewise a successful no-op even when a retry names different artifact bindings.
...
Approval PRs do not edit shared release state. One version-2 Flight dispatch produces at most one review PR containing the successfully admitted subset; merging that PR atomically blesses every record it contains. Retried, failed, or unavailable members do not block this subset and can be covered by a later Flight run. Legacy single-candidate dispatches produce one approval PR each. Separate approval PRs with disjoint targets can still be reviewed and merged in any order without rebasing or refreshing each other. Staleness is scoped to the records a request actually changes: publication requires each target's current hash to equal the prior hash recorded at approval. Two approvals for the same target are rejected as overlapping and must be published sequentially, because their order changes the meaning of the second review.
