...
The completion job writes Flight's version-2 [`reference-image-lock.schema.json`](../schemas/reference-image-lock.schema.json) shape to `scripts/reference-image-lock.json` and removes every request fulfilled by the batch in the same PR. If a Flight lock-update PR is already open, later releases replace that bot-owned branch from the current Flight base instead of replaying its old deletion commits. Every request in the current release is reconciled: byte-identical released requests are removed again, historical requests changed after review remain pending, and every newly fulfilled request must exist with the exact checksum recorded at intake. The update uses a force-with-lease against the observed bot-branch head, so concurrent movement fails instead of being overwritten. Each pack entry pins both the encoded pack and an identity-keyed `images` map of decoded `pixelSha256` values, so Flight can decide whether a captured image is already represented without downloading the pack. Required `referenceImage` coverage stays in Flight throughout.
